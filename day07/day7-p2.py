...
def getSizeOfDeletable(node, needed):
    goodNodes = []
    for child in node.children:
        if child.isDir:
            subGoodNodes = getSizeOfDeletable(child, needed)
            goodNodes.append(subGoodNodes)
            subSize = getSize(child)
            if subSize >= needed:
                goodNodes.append(subSize)

    return 70000001 if len(goodNodes) == 0 else min(goodNodes)
...
neededSpace = 30000000 - (70000000 - getSize(root))

print(getSizeOfDeletable(root, neededSpace))
